...
const notSubscribedMsg = "Suscríbete a la notificación de terremoto para ver la historia de los terremotos.";
...
class Earthquakes extends React.Component {
  ...
  getInfo(data) {
    ...
    return `${region} • ${moment(data.report_time).format('YYYY-MM-DD • HH.mm.ss')}`;
  }
    ...
    if (this.props.topic[_item.topic_name]) {
      return (
        <View style={styles.listItem}>
          <View style={[styles.intensityStyle, { backgroundColor: _item.predicted_intensity_rgb }]}>
            {intensityTag[_item.predicted_intensity_tag]?
              <Label fontSize={5} style={styles.intensityValueStyle}>{intensityTag[_item.predicted_intensity_tag]}</Label>
             :<Image style={styles.logoImageStyle} source={require(logo)} />
            }
              </View>
          <View style={styles.infoContainerStyle}>
            <Label fontSize={4} style={styles.title}>{_item.predicted_intensity_tag}</Label>
...
const mapStateToProps = ({ FirebaseReducer }) => {
  ...
  return { loading, historicalEarthquakeLoading, topic, historicalEarthquake } = FirebaseReducer;
};

export default connect(mapStateToProps, { getHistoricalEarthquakes })(Earthquakes);
